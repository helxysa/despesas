import {
  ...
  // Registrar um gasto da receita mensal (para depósitos em cofrinhos)
  export const registrarGastoReceita = async (userId: string, valor: number, descricao: string) => {
    try {
      // Obter a receita atual do usuário
      const receita = await obterReceita(userId);

      if (!receita) {
        throw new Error("Receita não encontrada para este usuário");
      }

      // Adicionar como uma despesa mensal especial
      await addDoc(collection(db, "despesasMensais"), {
        userId,
        nome: `Depósito em cofrinho: ${descricao}`,
        valor,
      ...
      console.error("Erro ao registrar gasto da receita:", error);
      throw new Error("Não foi possível registrar o gasto da receita. Tente novamente.");
    }
  };
